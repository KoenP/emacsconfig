...
function createLanguageServer(connection, runtimeEnv, languageConfigs = {
    ...
    connection.onInitialize((params) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (options.documentFeatures) {
            const documentService = languageConfigs.getDocumentService(ts, configHost, runtimeEnv.fileSystemProvide, loadCustomPlugins(folders[0]));
            (yield Promise.resolve().then(() => require('./features/documentFeatures'))).register(connection, documents, documentService, options.documentFeatures.allowedLanguageIds);
        ...
        if (options.languageFeatures) {
            ...
            projects = (0, projects_1.createProjects)(runtimeEnv, languageConfigs, ts, tsLocalized, options, documents, connection, configHost, () => (0, inferredCompilerOptions_1.getInferredCompilerOptions)(ts, configuration), params.capabilities);
    ...
    connection.onInitialized(() => {
        ...
        connection.workspace.onDidChangeWorkspaceFolders(e => {
            const added = e.added.map(folder => vscode_uri_1.URI.parse(folder.uri)).filter(uri => uri.scheme === 'file').map(uri => uri.fsPath);
            const removed = e.removed.map(folder => vscode_uri_1.URI.parse(folder.uri)).filter(uri => uri.scheme === 'file').map(uri => uri.fsPath);
