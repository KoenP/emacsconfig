...
exports.createSnapshots = exports.combineContinuousChangeRanges = void 0;
...
class IncrementalScriptSnapshot {
            ...
            const snapshot = {
                ...
                getChangeRange: (oldSnapshot) => {
                    if (!cache.has(oldSnapshot)) {
                        const start = this.changes.findIndex(change => { var _a; return ((_a = change.snapshot) === null || _a === void 0 ? void 0 : _a.deref()) === oldSnapshot; }) + 1;
    ...
    clearUnReferenceVersions() {
        ...
        for (let i = 0; i <= this.changes.length - 2; i++) {
            ...
            if (change.version !== lastVersion && !((_a = change.snapshot) === null || _a === void 0 ? void 0 : _a.deref())) {
    ...
    applyVersionToRootDocument(version, removeBeforeVersions) {
        let removeEnd = -1;
...
function createSnapshots(connection) {
    const snapshots = shared.createPathMap();
    const onDidOpens = [];
    const onDidChangeContents = [];
    const onDidCloses = [];
    ...
    return {
        data: snapshots,
        onDidOpen: (cb) => onDidOpens.push(cb),
        onDidChangeContent: (cb) => onDidChangeContents.push(cb),
        onDidClose: (cb) => onDidCloses.push(cb),
    };
}
