...
function baseParse(pugCode) {
    ...
    function visitNode(node, next, parent) {
        if (node.type === 'Block') {
            ...
            codeGen.addCode2(node.val, getDocOffset(node.line, node.column), undefined);
        }
    }
    function addStartTag(node, selfClosing) {
        ...
        if (pugCode.substring(tagRange.start, tagRange.end) === node.name) {
            codeGen.addCode2(node.name, tagRange.start, undefined);
        }
        else {
        ...
        for (const attr of noTitleAttrs) {
            ...
            if (typeof attr.val !== 'boolean') {
                codeGen.addText('=');
                codeGen.addCode2(attr.val, getDocOffset(attr.line, attr.column), undefined);
            }
        }
        if (attrsBlock) {
            codeGen.addText(' ');
            codeGen.addCode2(attrsBlock.text, attrsBlock.offset, undefined);
        }
        if (selfClosing) {
            codeGen.addText(' />');
        }
    ...
    function addClassesOrStyles(attrs, attrName) {
        ...
        for (const attr of attrs) {
            if (typeof attr.val !== 'boolean') {
                codeGen.addText(' ');
                codeGen.addCode2(attr.val.slice(1, -1), // remove "
                getDocOffset(attr.line, attr.column + 1), undefined);
            }
        }
        codeGen.addText('"');
    }
